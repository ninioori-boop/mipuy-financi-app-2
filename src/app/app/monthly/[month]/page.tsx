  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-xl border border-line bg-surface2 p-5 space-y-3">
        ...
        <div className="grid grid-cols-[1fr_6rem_6rem_5rem_5rem_1.5rem] gap-2 px-1 text-xs text-muted-txt font-medium">
          <span>שם העסקה</span>
          <span className="text-left">סכום כולל ₪</span>
          <span className="text-left">חודשי ₪</span>
          <span className="text-center">תשלום נוכחי</span>
          <span className="text-center">סה&quot;כ תשלומים</span>
          <span />
        </div>
        <div className="space-y-1">
          {data.installments.map(row => (
            <div key={row.id} className="grid grid-cols-[1fr_6rem_6rem_5rem_5rem_1.5rem] gap-2 items-center group">
              <input value={row.name} onChange={e => updateInstRow(monthId, row.id, 'name', e.target.value)}
              ...
              <input type="number" value={row.total || ''} onChange={e => updateInstRow(monthId, row.id, 'total', parseFloat(e.target.value) || 0)}
              ...
              <input type="number" value={row.monthly || ''} onChange={e => updateInstRow(monthId, row.id, 'monthly', parseFloat(e.target.value) || 0)}
                placeholder="₪" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-left tabular-nums" />
              <input type="number" value={row.current || ''} onChange={e => updateInstRow(monthId, row.id, 'current', parseFloat(e.target.value) || 0)}
                placeholder="#" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-center tabular-nums" />
              <input type="number" value={row.totalPay || ''} onChange={e => updateInstRow(monthId, row.id, 'totalPay', parseFloat(e.target.value) || 0)}
                placeholder="#" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-center tabular-nums" />
              <button onClick={() => deleteInstRow(monthId, row.id)}
        ...
        <button onClick={() => addInstRow(monthId)} className="text-xs text-muted-txt hover:text-gold transition-colors">
          + הוסף עסקה
        </button>
      </div>
      ...
      <div className="rounded-xl border border-line bg-surface2 p-5 space-y-3">
        ...
        <div className="grid grid-cols-[1fr_7rem_7rem_6rem_1.5rem] gap-2 px-1 text-xs text-muted-txt font-medium">
          <span>שם הנושה / הלוואה</span>
          <span className="text-left">יתרה לסגירה ₪</span>
          <span className="text-left">החזר חודשי ₪</span>
          <span className="text-center">חודשים</span>
          <span />
        </div>
        <div className="space-y-1">
          {data.debts.map(row => (
            <div key={row.id} className="grid grid-cols-[1fr_7rem_7rem_6rem_1.5rem] gap-2 items-center group">
              <input value={row.name} onChange={e => updateDebtRow(monthId, row.id, 'name', e.target.value)}
                placeholder="שם הנושה"
                className="rounded-lg border border-line bg-surface px-3 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60" />
              <input type="number" value={row.remaining || ''} onChange={e => updateDebtRow(monthId, row.id, 'remaining', parseFloat(e.target.value) || 0)}
                placeholder="₪" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-left tabular-nums" />
              <input type="number" value={row.monthly || ''} onChange={e => updateDebtRow(monthId, row.id, 'monthly', parseFloat(e.target.value) || 0)}
                placeholder="₪" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-left tabular-nums" />
              <input type="number" value={row.months || ''} onChange={e => updateDebtRow(monthId, row.id, 'months', parseFloat(e.target.value) || 0)}
                placeholder="#" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-center tabular-nums" />
              <button onClick={() => deleteDebtRow(monthId, row.id)}
                className="text-muted-txt hover:text-expense transition-colors opacity-0 group-hover:opacity-100 text-sm leading-none">×</button>
            </div>
          ))}
          {data.debts.length === 0 && <p className="text-xs text-muted-txt py-2">אין החזרי חובות</p>}
        </div>
        <button onClick={() => addDebtRow(monthId)} className="text-xs text-muted-txt hover:text-gold transition-colors">
          + הוסף חוב
        </button>
      </div>
      ...
      <div className="rounded-xl border border-line bg-surface2 p-5 space-y-3">
        ...
        <div className="grid grid-cols-[1fr_8rem_8rem_1.5rem] gap-2 px-1 text-xs text-muted-txt font-medium">
          <span>שם / סוג חיסכון</span>
          <span className="text-left">הפרשה חודשית ₪</span>
          <span className="text-left">סך נצבר ₪</span>
          <span />
        </div>
        <div className="space-y-1">
          {data.savings.map(row => (
            <div key={row.id} className="grid grid-cols-[1fr_8rem_8rem_1.5rem] gap-2 items-center group">
              <input value={row.name} onChange={e => updateSavingRow(monthId, row.id, 'name', e.target.value)}
                placeholder="חיסכון"
                className="rounded-lg border border-line bg-surface px-3 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60" />
              <input type="number" value={row.monthly || ''} onChange={e => updateSavingRow(monthId, row.id, 'monthly', parseFloat(e.target.value) || 0)}
                placeholder="₪" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-left tabular-nums" />
              <input type="number" value={row.accumulated || ''} onChange={e => updateSavingRow(monthId, row.id, 'accumulated', parseFloat(e.target.value) || 0)}
                placeholder="₪" min={0} style={{ direction: 'ltr' }}
                className="rounded-lg border border-line bg-surface px-2 py-1.5 text-sm text-txt placeholder:text-muted-txt focus:outline-none focus:border-gold/60 text-left tabular-nums" />
              <button onClick={() => deleteSavingRow(monthId, row.id)}
                className="text-muted-txt hover:text-expense transition-colors opacity-0 group-hover:opacity-100 text-sm leading-none">×</button>
            </div>
          ))}
          {data.savings.length === 0 && <p className="text-xs text-muted-txt py-2">אין הפרשות חיסכון</p>}
        </div>
        <button onClick={() => addSavingRow(monthId)} className="text-xs text-muted-txt hover:text-gold transition-colors">
          + הוסף חיסכון
        </button>
      </div>
      ...
      <div className="rounded-xl border border-line bg-surface2 p-5 space-y-4">
        ...
        <div className="grid grid-cols-[1fr_8rem_8rem] gap-x-3 gap-y-2">
          {/* Header */}
          <span />
