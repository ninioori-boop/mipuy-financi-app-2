    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="rounded-xl border border-line bg-surface2 p-5 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          {activeMonths > 0 && (
            <span className="text-xs text-muted-txt">{activeMonths} חודשים עם נתוני ביצוע</span>
          )}
        </div>
            ...
            <tbody className="divide-y divide-line/40">
              {[
                ...
                <tr key={label} className="hover:bg-surface/30">
                  ...
                  {moActuals.map((m, i) => {
                    ...
                    if (view === 'plan') return <td key={i} className="py-2 px-1 text-center text-gold/80 tabular-nums">{plan > 0 ? fmt(plan) : '—'}</td>
                    if (view === 'actual') return <td key={i} className={`py-2 px-1 text-center tabular-nums ${act > 0 ? 'text-green-400' : 'text-muted-txt'}`}>{act > 0 ? fmt(act) : '—'}</td>
                    return (
              ...
              <tr className="border-t-2 border-line">
                ...
                {moActuals.map((m, i) => {
                  ...
                  const val = view === 'actual' ? (m.hasData ? actNet : null) : planNet
