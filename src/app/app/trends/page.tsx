  ...
  if (rows.length === 0) {
    return (
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="rounded-xl border border-line bg-surface2 p-14 text-center">
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-xl border border-line bg-surface2 p-4">
          ...
          <div className="text-xl font-black text-green-400">{fmt(ytdIncome)}</div>
        </div>
        <div className="rounded-xl border border-line bg-surface2 p-4">
          <div className="text-xs text-muted-txt font-medium mb-1">סך הוצאות YTD</div>
          <div className="text-xl font-black text-expense">{fmt(ytdExpenses)}</div>
        </div>
        <div className="rounded-xl border border-line bg-surface2 p-4">
          <div className="text-xs text-muted-txt font-medium mb-1">ממוצע תזרים חודשי</div>
          <div className={`text-xl font-black ${avgCashflow >= 0 ? 'text-green-400' : 'text-expense'}`}>
            {avgCashflow >= 0 ? '+' : '−'}{fmt(avgCashflow)}
          </div>
        </div>
        <div className="rounded-xl border border-line bg-surface2 p-4">
          <div className="text-xs text-muted-txt font-medium mb-1">שיעור חיסכון</div>
          <div className="text-xl font-black text-gold">{savingsRate}%</div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-line bg-surface2 p-5 space-y-4">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={incomeExpData} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
            ...
            <YAxis tick={{ fill: '#8A8178', fontSize: 11 }} axisLine={false} tickLine={false}
