...
export function BudgetSection({ title, icon, rows, isIncome = false, onAdd, onUpdate, onDelete }: Props) {
  ...
  return (
    <div className="rounded-xl border border-line bg-surface2 p-5 space-y-3">
      ...
      {/* Column headers */}
      <div className="grid grid-cols-[1fr_3.8rem_3.8rem_1rem] sm:grid-cols-[1fr_6rem_6rem_1.5rem] gap-2 px-1 text-xs text-muted-txt font-medium">
        <span>פריט</span>
      ...
      <div className="space-y-1">
        {rows.map(row => (
          <div key={row.id} className="grid grid-cols-[1fr_3.8rem_3.8rem_1rem] sm:grid-cols-[1fr_6rem_6rem_1.5rem] gap-2 items-center group">
            <input
              value={row.name}
