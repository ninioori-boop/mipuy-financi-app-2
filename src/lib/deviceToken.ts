...
export function verifyDeviceToken(token: string, secret: string): { uid: string; version: number } | null {
  ...
  if (!uid) return null
  // The two formats sign overlapping message spaces: v0 signs `uid`, v>=1 signs
  // `uid:version`. Without this check an attacker can take a v3 token, re-encode
  // its SIGNED MESSAGE ("uid:3") as the uid of a 2-part token, reuse the same MAC
  // — and the result verifies as a v0 token for the phantom uid "uid:3". That uid
  // has no deviceTokens doc, so the revocation check passes and a revoked phone
  // keeps full access (and gets fresh rate-limit/quota buckets). Firebase Auth
  // uids are alphanumeric, so no legitimate token can contain ':'.
  ...
  if (parts.length === 3) {
    ...
    // Canonical form only: "03", "+3", "3e0" and " 3" all Number() to 3, which
    // would give one credential several valid spellings. One token, one string.
    if (String(version) !== parts[1]) return null
    message = `${uid}:${version}`
    mac = parts[2]
  }
