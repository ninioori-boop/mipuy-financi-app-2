 ...
 *
 * `declared` flips what this block IS. The questionnaire now asks for net income
 * per earner per month, so when that was filled in, the deposits are no longer
 * the source of the income section — they are the same money arriving, and the
 * only honest thing to do with them is check them against what was declared.
 * The header says so, because a block labelled "הכנסות" alongside a declared
 * figure is an invitation to add the two together.
 */
export function formatIncomeBreakdown(lines: NamedTotal[], months = 1, declared = false): string[] {
  if (!lines.length) return []
  const m = Math.max(1, months)
  const out = declared
    ? [
        '🔴 ההכנסה של המיפוי נמסרה בשאלון. הרשימה כאן היא ההפקדות שנצפו בעו"ש, והיא לאימות בלבד.',
        'ההפקדה של המשכורת והשכר שנמסר בשאלון הם אותו כסף. אל תוסיף שורות מהרשימה הזאת ל‑income, ואל תסכם את שתיהן.',
        'השתמש בה רק לשני דברים: לזהות מקור הכנסה שנכנס לחשבון ולא נמסר בשאלון, ולציין ב‑assessment פער בין המוצהר למה שהגיע בפועל.',
...
- **savings (חיסכון ונכסים)**: הפקדות לחיסכון/פנסיה/קרנות, **וגם כל נכס ששוויו ידוע**.
...
- **בלוק ההכנסות** — הפקדות שזוהו בעו"ש, כבר כממוצע חודשי.
  🔴 **סדר העדיפות של ההכנסה: מה שנמסר בשאלון קודם להפקדות.** אם בבלוק השאלון יש "הכנסה נטו שנמסרה" — **זה סעיף income, כמו שהוא**, שורה לכל משלם. בלוק ההפקדות אז הוא אימות בלבד, וכתוב בראשו שהוא כזה. רק כשלא נמסרה הכנסה בשאלון, ההפקדות הן המקור לסעיף.
  🔴 **הכנסה שנמסרה וההפקדה שלה בעו"ש הן אותו כסף. שורה אחת, לא שתיים, ולא סכום של השתיים.** אל תיצור "משכורת (מהשאלון)" ליד "משכורת (מהבנק)". אותו כלל לתלוש שכר ולכל מסמך שמתאר הכנסה שכבר נראית בעו"ש: המסמך מסביר **מה** נכנס, וההפקדה מראה שהוא אכן נכנס.
  אם הפקדה גדולה בעו"ש אינה מופיעה כלל בשאלון — היא כן ראויה לשורה, כי היא הכנסה שלא הוצהרה. ציין את זה ב‑source.
...
- **בלוק "גיליון מצורף"** — טבלה גולמית מקובץ אקסל שאינו דוח עו"ש ואינו דוח אשראי. בעיקר **לוח סילוקין**, ולפעמים מסמך אחר שהלקוח צירף. **קרא אותה כטבלה.** הכותרת האמיתית לא תמיד בשורה הראשונה — יש בקבצים האלה שורות כותרת עליונות, שורות ריקות ולפעמים שתי טבלאות בזה אחר זה. זהה את שורת הכותרת לפי התוכן, והתעלם משורות שאינן נתונים. הסכומים בגיליון הם **כפי שהם**, לא חולקו במספר החודשים.

### לוח סילוקין — איך קוראים אותו
לוח סילוקין הוא המסמך היחיד שנשאר בשאלון, כי הוא היחיד שאי אפשר להחליף בהקלדה. הוא מגיע כאקסל, PDF או צילום, והוא בנוי כטבלה של תשלום אחרי תשלום: מספר תשלום, תאריך, החזר חודשי, פירוק לקרן ולריבית, ו**יתרת הלוואה** שיורדת משורה לשורה.
מה לחלץ ממנו לשורה ב‑debts:
- **remainingBalance** — יתרת ההלוואה בשורה של **התשלום הבא שטרם שולם**, לא היתרה בשורה הראשונה ולא סכום ההלוואה המקורי.
- **monthlyPayment** — ההחזר החודשי בשורות העתידיות. אם הוא משתנה משורה לשורה (הלוואת גרייס, ריבית משתנה), קח את התשלום הקרוב.
- **interestRate** — הריבית **השנתית** הנקובה בכותרת הלוח. אם רשומה רק ריבית חודשית, הכפל ב‑12. אל תגזור ריבית מהחישוב שלך.
- **remainingMonths** — כמה שורות תשלום נותרו מהיום ועד סוף הלוח.
- **originalBalance** — סכום ההלוואה המקורי, אם הוא מופיע.
🔴 שדה שלא מצאת בלוח נשאר 0. אל תחשב אותו ואל תשלים אותו מהיגיון — יתרה מומצאת נראית בדיוק כמו יתרה אמיתית, ואף אחד לא יתפוס אותה.
🔴 לוח סילוקין הוא **מסמך של הלוואה אחת**. אל תאחד שני לוחות לשורה אחת, ואל תפצל לוח אחד לשתי הלוואות.
...
### מספרים שהוקלדו — הכי מוסמכים שיש
חלק מהשאלון הוא טבלאות שממלאים ביד: ההכנסה נטו של כל מפרנס בשלושת החודשים האחרונים, הקופות והקרנות מהר הכסף, ותיקי ההשקעות והנכסים. הן מגיעות אליך כשורות מוכנות, **והמספר בהן כבר מעובד לעמודה שלו** — הממוצע החודשי של ההכנסה כבר חושב, ואין שום דבר לחשב מחדש.
🔴 **כל שורה בטבלאות האלה חייבת להופיע במיפוי.** אלה לא הסקות מדוח, אלה מספרים שאדם ישב והקליד. סכום שהוקלד ולא מופיע בשום מקום במיפוי הוא נכס שנעלם, וזו התקלה שהטבלאות האלה נולדו כדי למנוע. אזכור ב‑assessment אינו תחליף לשורה.
🔴 **ואל תספור אותם פעמיים.** שכר שנמסר בטבלה מופיע גם כהפקדה בעו"ש; תיק השקעות שנמסר עשוי להופיע גם בצילום מסך. זה אותו כסף: שורה אחת, על פי המספר שהוקלד.
