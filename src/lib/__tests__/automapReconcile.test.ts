...
describe('checkIncomeAgainstDeposits', () => {
  ...
  it('flags income counted twice, and names the declared figure as the likely cause', () => {
    const c = checkIncomeAgainstDeposits(withIncome(28000), { deposits: 42000, months: 3, hasDeclaredIncome: true })!
    expect(Math.round(c.depositsPerMonth)).toBe(14000)
    expect(Math.round(c.excessPerMonth)).toBe(14000)
    expect(c.detail).toContain('נמסרה הכנסה בשאלון')
    expect(c.detail).toContain('אותו כסף')
  })

  it('says something different when no income was declared', () => {
    const c = checkIncomeAgainstDeposits(withIncome(28000), { deposits: 42000, months: 3, hasDeclaredIncome: false })!
    expect(c.detail).toContain('חשבון אחר')
    expect(c.detail).not.toContain('נמסרה הכנסה בשאלון')
  })

  it('is silent when the mapping matches the deposits', () => {
    expect(checkIncomeAgainstDeposits(withIncome(14000), { deposits: 42000, months: 3, hasDeclaredIncome: true })).toBeNull()
  })
  ...
  it('never flags income lower than the deposits', () => {
    expect(checkIncomeAgainstDeposits(withIncome(9000), { deposits: 42000, months: 3, hasDeclaredIncome: true })).toBeNull()
  })

  it('ignores a difference that is only rounding', () => {
    expect(checkIncomeAgainstDeposits(withIncome(14400), { deposits: 42000, months: 3, hasDeclaredIncome: true })).toBeNull()
  })

  it('needs BOTH a material share and a material amount', () => {
    // 40% over, but only ₪400/month: too small to chase.
    expect(checkIncomeAgainstDeposits(withIncome(1400), { deposits: 3000, months: 3, hasDeclaredIncome: true })).toBeNull()
  })
  ...
  it('counts business receipts as income — they arrive in the same account', () => {
    ...
    expect(checkIncomeAgainstDeposits(biz, { deposits: 42000, months: 3, hasDeclaredIncome: false })).not.toBeNull()
  })

  it('says nothing with no bank data to compare against', () => {
    expect(checkIncomeAgainstDeposits(withIncome(28000), { deposits: 0, months: 3, hasDeclaredIncome: true })).toBeNull()
  })
})
