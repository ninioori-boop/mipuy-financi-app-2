...
| `shared/learnedDB` | מילון הסיווג המשותף. 🔴 מהאפליקציה נכתב **רק** דרך `/api/learn`, ורק אחרי אישור של שני לקוחות. יש דלת שנייה: `scripts/review-ai-suggestions.ts --apply`, כלי מפעיל עם סקירה אנושית שעוקף את חוק שני הלקוחות |
| `learnedProposals/{hash}` | הצעות שטרם אושרו לכניסה למאגר המשותף. שרת בלבד (admin SDK), חסום לדפדפן ע"י ה-catch-all בכללים. מזהי הלקוחות **מגובבים**, כדי שמחיקת חשבון לא תשאיר כאן שארית |
...
🔴 shared/learnedDB נכתב מהאפליקציה אך ורק דרך /api/learn, ו-learnedSharing.ts
   חוסם ממנו אמצעי תשלום ("ביט", "פייפאל", "העברה", "משיכה") וקטגוריות אישיות
   כמו "ביט ללא מעקב". בלי החסימה הזאת כל הלקוחות לומדים שהעברה בביט היא
   קטגוריה מסוימת. זה קרה, וניקוי ידני היה הפתרון.
   מאז 12/08/2026 יש שכבה שנייה: תיקון נכנס למאגר רק אחרי ש**שני משקי בית
   שונים** הגיעו לאותה תשובה (learnedQuorum.ts). מה שזה **לא** מבטיח: יועץ
   שמתקן אותו דבר אצל שני לקוחות שלו מספק את שני הקולות בעצמו. זה מכוון, וזה
   הכלל שאורי אישר. השכבה מונעת שקול אחד יהפוך לעובדה, לא ששגיאה עקבית של
   מומחה תתפשט.
   ⚠️ יש דלת אחת שעוקפת את חוק שני הלקוחות: scripts/review-ai-suggestions.ts
   עם --apply. היא כלי מפעיל עם סקירה אנושית, לא מסלול אפליקציה.
...
- **מתג החירום של ה-AI נכשל פתוח, אבל רק בהתנעה קרה.** מאז 11/08/2026 כשל קריאה
  מ-Firestore **שומר** על המצב האחרון שנקרא בהצלחה במקום להתאפס ל-`false`, ומדפיס
  אזהרה. כלומר מתג שכבר נצפה דלוק נשאר דלוק גם כשהתשתית מתנדנדת. מה שנשאר פתוח:
  אינסטנס שמעולם לא הצליח לקרוא (deploy חדש בזמן תקלה) עדיין מאפשר להמשיך, בכוונה,
  כדי לא להשבית את המערכת מעצמה. הלוגיקה כפולה: `src/lib/aiBudget.ts` ו-`functions/clientBot.js`.
